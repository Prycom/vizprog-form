...
export default function Form() {
  ...
  const handleSubmitForm = async (e) => {
    ...
    if (state.contentForms.length === state.inputs.length) {
      ...
      try {
        ...
        alert("Введенные вами данные были успешно записаны");
        push("/");
      } catch (e) {
        alert(e);
      } finally {
      ...
      alert("пропусков быть нге должно!");
    }
  };
  ...
  const handleEditForm = async (e) => {
    ...
    if (state.titleForm.title) {
      ...
      alert("Заголовок формы не может быть пустым");
    }
  };
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        ...
        console.error("данные недоступны");
      } finally {
        dispatch({ type: "CHANGE_ISLOADING", value: false });
      }
    };
